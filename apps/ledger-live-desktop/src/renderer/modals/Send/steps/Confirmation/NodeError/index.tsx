...
const NodeError: React.FC<Props> = ({ currencyName, networkName, error }) => {
  const theme = useTheme();
  const { t } = useTranslation();
  const { handleExportLogs } = useExportLogs();

  const transactionBroadcastError = createTransactionBroadcastError(error);
  const url = transactionBroadcastError.url ?? urls.contactSupport;
        ...
        <ErrorBody
          title={
            <Text variant="h3Inter" fontSize={24} textAlign="center">
              <TranslatedError error={transactionBroadcastError} />
            </Text>
          }
          description={
      ...
      <Flex flexDirection="column" rowGap={16} width="100%" alignItems="flex-start">
        ...
        {isShowMore && (
          <Grid columns={2} columnGap="8px" width="100%">
            ...
            <TechnicalErrorSection error={transactionBroadcastError} onSaveLogs={onSaveLogs} />
          </Grid>
        )}
      </Flex>
    </Flex>
